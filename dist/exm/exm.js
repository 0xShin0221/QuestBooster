...
var require_helpers = __commonJS({
  "contracts/common/lib/jsonschema/helpers.js"(exports, module) {
    ...
    exports.isFormat = function isFormat(input, format, validator3) {
      ...
      } else if (validator3 && validator3.customFormats && typeof validator3.customFormats[format] === "function") {
        return validator3.customFormats[format](input);
      }
      return true;
    };
    ...
    Validator.prototype.validateSchema = function validateSchema2(instance, schema, options, ctx) {
      ...
      for (var key in schema) {
        if (!attribute.ignoreProperties[key] && skipAttributes.indexOf(key) < 0) {
          ...
          if (validator3) {
            validatorErr = validator3.call(this, instance, schema, options, ctx);
...
// contracts/common/actions/read/version.js
...
// contracts/common/exm/actions/read/copy.js
var copy = async (state, action) => {
  if (state.canEvolve !== true)
    err("contract cannot evolve");
  return {
    result: state
  };
};
...
// contracts/common/exm/actions/write/evolve.js
var evolve3 = async (state, action, signer) => {
  ...
  if (state.canEvolve) {
    const prev_state = (await EXM.deterministicFetch(
    ...
    if (isNil_default(prev_state) || prev_state.success !== true) {
      err("The previous contract doesn't allow evolve.");
    }
    let _state = mergeLeft_default(prev_state.result, state);
...
export async function handle(state, action) {
  ...
  switch (action.input.function) {
    ...
    case "add":
      await add3(_state, action);
      break;
    case "set":
      await set3(_state, action);
      break;
    case "update":
      await update3(_state, action);
      break;
    case "upsert":
      await upsert(_state, action);
      break;
    case "delete":
      await remove3(_state, action);
      break;
    case "batch":
      await batch(_state, action);
      break;
    case "addIndex":
      await addIndex4(_state, action);
      break;
    case "removeIndex":
      await removeIndex2(_state, action);
      break;
    case "setSchema":
    ...
    case "setCanEvolve":
      ...
      break;
    case "evolve":
      _state = (await evolve3(_state, action)).state;
      break;
    case "addOwner":
